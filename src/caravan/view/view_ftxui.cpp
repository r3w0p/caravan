...
namespace Caravan::View {
    ...
    void push_card(ViewFTXUI *config, ftxui::Elements *e, Model::Card card, bool lead) {
        using namespace ftxui;
    ...
    std::shared_ptr<ftxui::Node> gen_card(ViewFTXUI *config, Model::Card card, bool hide, bool highlight, bool blank = false) {
        using namespace ftxui;
        ...
        if(blank) {
            ...
            ret = ret | borderHeavy | (config->wants_colour() ? color(Color::Palette16::MagentaLight) : color(Color::Default));
        } else {
    ...
    std::shared_ptr<ftxui::Node> gen_faces(ViewFTXUI *config, const Model::Slot &slot, const bool blank = false) {
        using namespace ftxui;
    ...
    std::shared_ptr<ftxui::Node> gen_caravan(ViewFTXUI *config, Model::Game *game, Model::CaravanName cn, bool top) {
        using namespace ftxui;
        std::shared_ptr<Node> ret;
        ...
        for (uint8_t i = 0; i < Model::TRACK_NUMERIC_MAX; i++) {
            if ((top && (Model::TRACK_NUMERIC_MAX - i) <= caravan_size) || (!top && i + 1 <= caravan_size)) {
        ...
        title.push_back(text(caravan_name_to_wstr(cn, true) + L" ") | maybe_colour);
        if (game->get_table()->get_caravan(cn)->get_size() > 0) {
            ...
            title.push_back(suit_to_text(caravan->get_suit(), config->wants_colour()));
            title.push_back(text(L")") | maybe_colour);
            title.push_back(text(L" "));
        }
        ...
        if(highlight) {
            ret = ret | (config->wants_colour() ? color(Color::Palette16::MagentaLight) : color(Color::Default));
        }

        return ret;
    }

    std::shared_ptr<ftxui::Node> gen_deck_card(ViewFTXUI *config, Model::Game *game, uint8_t position, Model::Card card, bool hide, bool highlight, bool blank = false) {
        using namespace ftxui;
        return hbox({
                        blank || game->get_winner() != Model::NO_PLAYER ? gen_position_blank() : gen_position(position),
    ...
    std::shared_ptr<ftxui::Node> gen_deck(ViewFTXUI *config, Model::Game *game, bool top) {
        using namespace ftxui;
        ...
        Model::Player *player_this = game->get_player(top ? Model::PLAYER_DEF : Model::PLAYER_ABC);
        User::User &user_this = player_this->get_name() == config->get_user_abc().get_name() ? config->get_user_abc() : config->get_user_def();
        User::User &user_other = player_this->get_name() == config->get_user_abc().get_name() ? config->get_user_def() : config->get_user_abc();
        ...
        uint8_t hand_max = std::max(std::max(hand_size_abc, hand_size_def), Model::HAND_SIZE_MAX_POST_START);
        ...
        bool hide =
            ...
            (
                ...
                config->get_user_turn()->get_name() != player_this->get_name() &&
                (config->get_user_abc().is_human() && config->get_user_def().is_human())
            ) ||
            (
                game->get_winner() == Model::NO_PLAYER &&
                !user_this.is_human() &&
        ...
        content = vbox(e);
        title = " " + (top ? config->get_name_def() : config->get_name_abc()) + " (" + std::to_string(total_cards) + ") ";
    ...
    std::shared_ptr<ftxui::Node> gen_input(
        ...
        bool is_mid = !config->get_msg_main().empty() || !config->get_msg_important().empty();
        bool is_low = !config->get_msg_move_abc().empty() || !config->get_msg_move_def().empty();

        if (is_top) {
            e.push_back(separatorEmpty());
            e.push_back(
                hbox(separatorEmpty(), text(config->get_name_turn() + " > "), (*comp_user_input)->Render(), separatorEmpty()));
            e.push_back(separatorEmpty());
        ...
        if (is_mid) {
            ...
            if (!config->get_msg_main().empty()) {
                e.push_back(hbox(separatorEmpty(), paragraph(config->get_msg_main()), separatorEmpty()));
                e.push_back(separatorEmpty());
            }

            if (!config->get_msg_important().empty()) {
                e.push_back(hbox(separatorEmpty(), paragraph(config->get_msg_important()), separatorEmpty()));
                e.push_back(separatorEmpty());
            }
        ...
        if (is_low) {
            ...
            if (!config->get_msg_move_abc().empty()) {
                e.push_back(hbox(separatorEmpty(), hbox(config->get_msg_move_abc()), separatorEmpty()));
                e.push_back(separatorEmpty());
            }

            if (!config->get_msg_move_def().empty()) {
                e.push_back(hbox(separatorEmpty(), hbox(config->get_msg_move_def()), separatorEmpty()));
                e.push_back(separatorEmpty());
            }
        }
    ...
    std::shared_ptr<ftxui::Node> gen_game(
        ...
        return hbox({
                        vbox({  // GAME AREA
                                 ...
                                 hbox({  // TOP GAME AREA
                                          gen_caravan(config, game, Model::CARAVAN_D, true),
                                          separatorEmpty(),
                                          separatorEmpty(),
                                          gen_caravan(config, game, Model::CARAVAN_E, true),
                                          separatorEmpty(),
                                          separatorEmpty(),
                                          gen_caravan(config, game, Model::CARAVAN_F, true),
                                      }),  // top game area

                                 separatorEmpty(),

                                 hbox({  // BOTTOM GAME AREA
                                          gen_caravan(config, game, Model::CARAVAN_A, false),
                                          separatorEmpty(),
                                          separatorEmpty(),
                                          gen_caravan(config, game, Model::CARAVAN_B, false),
                                          separatorEmpty(),
                                          separatorEmpty(),
                                          gen_caravan(config, game, Model::CARAVAN_C, false),
                                      }),  // bottom game area
    ...
    ftxui::Elements get_move_description(ViewFTXUI *config) {
        ...
        if (config->get_move().option == Model::OPTION_PLAY) {
            ...
            if(config->get_move().hand.is_face_card() && config->get_move().pos_caravan > 0) {
                e.push_back(text(L" on "));
                push_card(config, &e, config->get_move().board, false);
            }

            e.push_back(text(L" on " + caravan_name_to_wstr(config->get_move().caravan_name, false) + L"."));
            ...
            e.push_back(text(L" cleared " + caravan_name_to_wstr(config->get_move().caravan_name, false) + L"."));
        }
            ...
            try {
                ...
                if(game.get_winner() != Model::NO_PLAYER) {
                    std::string name_winner =
                        game.get_winner() == user_abc.get_name() ? name_abc : name_def;
                ...
                if(user_turn->is_human()) {
                    ...
                    if((time_bot_end-time_bot_start) >= bot_delay_millis) {
                ...
                try {
                    if(confirmed) {
                        ...
                        move = ctrl->convert(raw_command, confirmed);
                    } else {
                        // An incomplete move that can be used to highlight
                        // areas of the board as a hint to the player
                        highlight = ctrl->convert(raw_command, confirmed);
                    }

                    raw_command = "";

                    switch (move.option) {
                        case Model::NO_OPTION:
                            break;
                        default:
