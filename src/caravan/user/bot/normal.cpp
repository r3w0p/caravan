...
namespace Caravan::User {
    ...
    uint8_t pos_card_numeral(Model::Player *player) {
    ...
    uint8_t numeral_rank_value(Model::Card card) {
        switch (card.rank) {
            case Model::ACE:
                return 1;
            case Model::TWO:
    ...
    std::string UserBotNormal::generate_move(
        ...
        Model::Player *me = game->get_player(name);
        ...
        Model::PlayerCaravanNames my_cvns = game->get_player_caravan_names(name);
        Model::PlayerCaravanNames opp_cvns = game->get_player_caravan_names(
            name == Model::PLAYER_ABC ? Model::PLAYER_DEF : Model::PLAYER_ABC);

        uint16_t my_move_count = me->get_moves_count();

        if (my_move_count < Model::MOVES_START_ROUND) {
        ...
        if(allow_clear) {
            for (uint8_t i_cvn = 0; i_cvn < Model::PLAYER_CARAVANS_MAX; i_cvn++) {
                Model::Caravan *cvn = table->get_caravan(my_cvns[i_cvn]);

                if (cvn->get_bid() > Model::CARAVAN_SOLD_MAX ||
                    cvn->get_size() == Model::TRACK_NUMERIC_MAX) {
        ...
        for (uint8_t pos_hand = 1; pos_hand <= my_hand_size; pos_hand++) {
            Model::Card c_hand = me->get_from_hand_at(pos_hand);

            if (c_hand.is_numeral_card() && allow_numeral) {
                // If numeral, look through caravans
                for (uint8_t i = 0; i < Model::PLAYER_CARAVANS_MAX; ++i) {
                    Model::Caravan *my_cvn = table->get_caravan(my_cvns[i]);
                    Model::Caravan *opp_cvn = table->get_caravan(opp_cvns[i]);
                    ...
                    if (my_cvn_bid >= Model::CARAVAN_SOLD_MIN &&
                        my_cvn_bid <= Model::CARAVAN_SOLD_MAX &&
                        (my_cvn_bid > opp_cvn_bid ||
                         opp_cvn_bid > Model::CARAVAN_SOLD_MAX)) { continue; }

                    uint8_t my_cvn_size = my_cvn->get_size();

                    // Skip caravan if full
                    if (my_cvn_size == Model::TRACK_NUMERIC_MAX) {
                        continue;
                    }
                    ...
                    Model::Slot slot_top = my_cvn->get_slot(my_cvn_size);
                    ...
                    bool not_bust =
                        (my_cvn_bid +
                         numeral_rank_value(c_hand)) <= Model::CARAVAN_SOLD_MAX;
                    ...
                    Model::Direction my_cvn_dir = my_cvn->get_direction();

                    // Card ascending with caravan and would not bust
                    if (my_cvn_dir == Model::ASCENDING &&
                    ...
                    if (my_cvn_dir == Model::DESCENDING &&
                        c_hand.rank < slot_top.card.rank &&
                        not_bust) {
                        return move_draft;
                        }
                ...
                uint8_t i_opp_cvn_most_cards = Model::PLAYER_CARAVANS_MAX;
                ...
                if (i_opp_cvn_most_cards < Model::PLAYER_CARAVANS_MAX &&
                    n_opp_cvn_most_cards > 0) {

                    Model::Caravan *opp_cvn = table->get_caravan(
                        opp_cvns[i_opp_cvn_most_cards]);

                    uint8_t opp_cvn_size = opp_cvn->get_size();
                    Model::Slot opp_slot_top = opp_cvn->get_slot(opp_cvn_size);
                    ...
                    if (opp_slot_top.i_faces < Model::TRACK_FACE_MAX) {
                        return move_draft;
                    }
                    }
            }
    ...
    std::string UserBotNormal::request_move(Model::Game *game) {
