...
Slot Table::get_slot_at(CaravanName cn, uint8_t pos) {
...
void Table::play_face_card(CaravanName cn, Card card, uint8_t pos) {
    ...
    if (card.rank == QUEEN and pos != cvn_target->get_size())
        throw CaravanGameException(
                "QUEEN on numeral card at the highest caravan pos." + std::to_string(pos));
...
void Table::play_numeral_card(CaravanName cn, Card c) {
    Caravan *caravan_ptr = caravans[caravan_name_to_index_value(cn)];
    caravan_ptr->put_numeral_card(c);
}
