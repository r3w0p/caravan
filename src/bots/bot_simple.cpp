...
GameOption UserBotSimple::request_option(Engine *e, View *v) {
    Player *p;
    uint8_t p_hand_size;
    Table *t;
        ...
        for (uint8_t pos = 1; pos <= p_hand_size; ++pos) {
            ...
            if (is_numeric_card(c_hand)) {
                for (uint8_t i = 0; i < PLAYER_CARAVANS_MAX; ++i) {
                    ...
                    slot_cvn = t->get_caravan_cards_at(pcns_me[i], size_cvn);
                ...
                slot_cvn = t->get_caravan_cards_at(pcns_opp[i_cvn_most_cards],
                                                   size_cvn);
